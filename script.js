...
function generateQuestion() {
  ...
  if (selectedOperations.length === 0) {
    ...
    questionElement.innerHTML = `
      <span class="question-symbol">Escolha pelo menos uma operação.</span>
    `;
    answerOptionsElement.innerHTML = "";
    messageElement.textContent = "Selecione uma ou mais operações para continuar.";
    return;
  }
...
function checkAnswer(selectedAnswer, selectedButton) {
  if (!gameStarted || currentAnswer === null) {
    messageElement.textContent = "Selecione uma operação para começar.";
    return;
  }

  if (answeredCurrentQuestion) {
    messageElement.textContent = "Gere uma nova pergunta para continuar.";
    return;
  }
    ...
    if (!gameStarted) {
      ...
      if (selectedOperations.length === 0) {
        questionElement.innerHTML = `
          <span class="question-symbol">Escolha pelo menos uma operação.</span>
        `;
        messageElement.textContent = "Selecione uma ou mais operações e clique em Nova pergunta.";
      } else {
        questionElement.innerHTML = `
          <span class="question-symbol">Tudo pronto! Clique em Nova pergunta para começar.</span>
        `;
        messageElement.textContent = "";
      }
      return;
...
newButton.addEventListener("click", generateQuestion);

questionElement.innerHTML = `
  <span class="question-symbol">Escolha as operações e clique em Nova pergunta.</span>
`;
answerOptionsElement.innerHTML = "";
messageElement.textContent = "";
